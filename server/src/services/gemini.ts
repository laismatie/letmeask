...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Transcreva o audio para o português brasileiro. Seja preciso e natural na transcrição. Mantenha a pontuação adequada e divida o texto em parágrafos quando for apropriado.',
            },
            {
                inlineData: {
                    mimeType,
...
export async function generateAnswer(question: string, transcription: string[]) {
    ...
    const prompt = `
        ...
        - Se a resposta não puder ser obtida com base no contexto, responda com Não possui informações suficientes para responder";
        - Seja objetivo;
